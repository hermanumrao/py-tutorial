...
def Register():
    Database()
    if USERNAME.get == "" or PASSWORD.get() == "" or FULLNAME.get() == "" or CONFIRM_PASSWORD.get() == "" :
        ...
        if cursor.fetchone() is not None:
            ...
            conn.commit()
            write_file(FULLNAME.get(),USERNAME.get())
            USERNAME.set("")
            PASSWORD.set("")
            FULLNAME.set("")
...
def write_file(b,c):
    fout=open("tch_list.csv",'a',newline='\n')
    fin=open("tch_list.csv",'r',newline='\n')
