...
def RegisterForm():
    ...
    lbl_class = Label(RegisterFrame, text="Semester", font=('arial',18), bd=18)
...
def Register():
    Database()
    if USERNAME.get == "" or PASSWORD.get() == "" or FULLNAME.get() == "" or CONFIRM_PASSWORD.get() == "" or DCLASS.get() == "":
    ...
    elif int(DCLASS.get()) not in range(1,9):
        lbl_result2.config(text="please enter semester (1-8)", fg="orange")
    else:
        ...
        if cursor.fetchone() is not None:
            ...
            conn.commit()
            write_file(FULLNAME.get(),USERNAME.get(),DCLASS.get())
            USERNAME.set("")
            PASSWORD.set("")
            FULLNAME.set("")
...
def write_file(b,c,e):
    fout=open("stu_list.csv",'a',newline='\n')
    fin=open("stu_list.csv",'r',newline='\n')
...
def stud_apply():
    print("done")
    found=False
    fin1=open(".cca_list.csv",'r',newline='\n')
    print(user_name)
    ...
    if found2==False:
        ...
        try:
            ...
            fin4.close()
            fout4.close()
        os.remove("participant.dat")
        os.rename("temp2.dat","participant.dat")
