...
def write_file(b,c,d,e,f):
    ...
    fin=open(".cca_list.csv",'r')
    fout=open(".cca_cache.txt",'r+')
    prticpnt=open("participant.dat",'ab')
    fin.seek(0)
    fout.seek(0)
...
def del_file():
    fout=open(".cca_list1.csv",'a')
    fin=open(".cca_list.csv",'r',newline='\n')
    rno=str(DELID.get())
    ...
    for i in read_obj:
        if i[0]==rno:
            ...
            print(i)
            csv_obj.writerow(i)
    os.remove(".cca_list.csv")
    os.rename(".cca_list1.csv",".cca_list.csv")

def fin_file():
    winners=[wwin.get(),w1.get(),w2.get(),w3.get(),w4.get()]
    print(winners)
    fin=open(".cca_list.csv",'r',newline='\n')
    fout=open("cca_fin.csv",'a')
    csv_obj=csv.writer(fout)
    read_obj=csv.reader(fin)
    for i in read_obj:
        print(i,winners)
